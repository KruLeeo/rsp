...
def update_statistics(endpoint, execution_time):
    # обновление статистики выполнения запросов
...
def measure_time(func):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            # Время считается ПОСЛЕ выполнения метода
            end_time = time.time()
            execution_time = end_time - start_time
            
            # Получаем имя эндпоинта
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                log_response_info(response, status_code, execution_time)
                return response, status_code
            else:
                log_response_info(result, 200, execution_time)
                return result
                
        except Exception as e:
            # В случае ошибки тоже считаем время выполнения
            end_time = time.time()
            execution_time = end_time - start_time
            endpoint = f"{request.method} {request.path}"
            update_statistics(endpoint, execution_time)
            logger.error(f"Ошибка в {func.__name__}: {str(e)} - Время выполнения: {execution_time:.10f} сек")
            raise
...
def get_books():
    ...
    try:
        ...
        logger.error(f"Ошибка при получении всех книг: {str(e)}")
...
def update_book(book_id):
    ...
    try:
        ...
        logger.error(f"Ошибка при обновлении книги {book_id}: {str(e)}")
        return jsonify({"error": "Внутренняя ошибка сервера"}), 500
...
def delete_book(book_id):
    ...
    try:
        ...
        logger.error(f"Ошибка при удалении книги {book_id}: {str(e)}")
        return jsonify({"error": "Внутренняя ошибка сервера"}), 500
...
@measure_time
def get_stats():
    """Новый эндпоинт для получения статистики выполнения запросов"""
    log_request_info()
    try:
        ...
        stats_summary = {
            ...
            'average_response_time_sec': round(request_stats['average_time'], 3),
            'endpoints': {}
        }
        ...
        for endpoint, endpoint_stat in request_stats['endpoints'].items():
            stats_summary['endpoints'][endpoint] = {
                ...
                'average_time_sec': round(endpoint_stat['average_time'], 3),
                'min_time_sec': round(endpoint_stat['min_time'], 3),
                'max_time_sec': round(endpoint_stat['max_time'], 3)
            }
